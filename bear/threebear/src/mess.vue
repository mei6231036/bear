...
<span class="input-icon">
	...
	<tbody>
		...
		<tr v-for="(val,index) in data">
			...
			<td>
				<span class="label label-sm label-warning">{{val.id}}</span>
			</td>
			<td>{{val.content}}</td>
			<td>{{$moment(1501251212311).format('YYYY-MM-DD HH:mm:ss')}}</td>
...
export default {
 ...
 data(){return{
 	...
 	mounted: function() {
			...
			this.$http.jsonp('http://www.bear.com/advanced/backend/web/index.php?r=official/show',{}, {
	        headers: {
...
// },
   sea:function()
   {
   	this.$http.jsonp('http://www.bear.com/advanced/backend/web/index.php?r=official/show',{params:{name:this.sear}}, {
	        headers: {
   ...
   page:function (page)
   {
   		...
   		this.$http.jsonp('http://www.bear.com/advanced/backend/web/index.php?r=official/show',{params:{page:page,name:this.sear}}, {
	        headers: {
