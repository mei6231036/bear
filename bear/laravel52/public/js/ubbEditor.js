function ubbEditor(_id)
{
  ...
  this.tHTMLClear = function(_strers)
  {
    ...
    if (tstrers)
    {
	    ...
	    tstrers = tstrers.replace(/<([a-z]+)+\s*(?:onerror|onload|onunload|onresize|onblur|onchange|onclick|ondblclick|onfocus|onkeydown|onkeypress|onkeyup|onmousemove|onmousedown|onmouseout|onmouseover|onmouseup|onselect)[^>]*>/gi, '<other/$1>');
    };
    return tstrers;
  };
  this.tHTML2XHTML = function(_strers)
  {
    ...
    if (tstrers)
    {
      ...
      tstrers = tstrers.replace(/\]/igm, '&#93;');
	    ...
	    tstrers = tstrers.replace(/(<hr\s+[^>]*[^\/])(>)/ig, 'other/$1 />');
	    tstrers = tstrers.replace(/(<img\s+[^>]*[^\/])(>)/ig, 'other/$1 />');
    };
    return tstrers;
  };
  this.tXHTML2UBB = function(_strers)
  {
    ...
    if (tstrers)
    {
      ...
      var tReplaceAry = [
        [/<br \/>/ig, '[br]', false],
        [/<p>([^<]*?)<\/p>/igm, '[p]other/$1[/p]', true],
        [/<b>([^<]*?)<\/b>/igm, '[b]other/$1[/b]', true],
        [/<strong>([^<]*?)<\/strong>/igm, '[b]other/$1[/b]', true],
        [/<i>([^<]*?)<\/i>/igm, '[i]other/$1[/i]', true],
        [/<em>([^<]*?)<\/em>/igm, '[i]other/$1[/i]', true],
        [/<u>([^<]*?)<\/u>/igm, '[u]other/$1[/u]', true],
        [/<ol>([^<]*?)<\/ol>/igm, '[ol]other/$1[/ol]', true],
        [/<ul>([^<]*?)<\/ul>/igm, '[ul]other/$1[/ul]', true],
        [/<li>([^<]*?)<\/li>/igm, '[li]other/$1[/li]', true],
        [/<span\s[^>]*?>([^<]*?)<\/span>/igm, function($0, other/$1) {
          var tString = other/$1;
        ...
        [/<font\s[^>]*?>([^<]*?)<\/font>/igm, function($0, other/$1) {
          var tString = other/$1;
        ...
        [/<div\s[^>]*?>([^<]*?)<\/div>/igm, function($0, other/$1) {
          var tString = other/$1;
  ...
  this.tUBB2XHTML = function(_strers)
  {
    ...
    if (tstrers)
    {
      ...
      var tReplaceAry = [
        ...
        [/\[ul\]([^\[]*?)\[\/ul\]/igm, '<ul>other/$1</ul>', true],
        [/\[li\]([^\[]*?)\[\/li\]/igm, '<li>other/$1</li>', true],
        [/\[code\]([^\[]*?)\[\/code\]/igm, '<div class="ubb_code" style="BORDER: #dcdcdc 1px dotted; PADDING: 5px; LINE-HEIGHT: 150%; FONT-STYLE: italic">other/$1</div>', true],
        [/\[quote\]([^\[]*?)\[\/quote\]/igm, '<div class="ubb_quote" style="BORDER: #dcdcdc 1px dotted; PADDING: 5px; LINE-HEIGHT: 150%">other/$1</div>', true],
        [/\[color=([^\]]*)\]([^\[]*?)\[\/color\]/igm, '<font style="color: other/$1">$2</font>', true],
        [/\[hilitecolor=([^\]]*)\]([^\[]*?)\[\/hilitecolor\]/igm, '<font style="background-color: other/$1">$2</font>', true],
        [/\[align=([^\]]*)\]([^\[]*?)\[\/align\]/igm, '<p align="other/$1">$2</p>', true],
        [/\[url=([^\]]*)\]([^\[]*?)\[\/url\]/igm, '<a href="other/$1">$2</a>', true],
        [/\[img\]([^\[]*?)\[\/img\]/igm, '<img src="other/$1" />', true]
      ];
