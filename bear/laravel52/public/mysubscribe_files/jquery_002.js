...
(function($) {
	...
	$.msgbox = function(opts) {
		...
		opts.width = (opts.width || 500) > $('body').width() ? $('body').width() : (opts.width || 500);
		// msgbox height, default 260px; $('body').height is set if height is greater than that.
		opts.height = (opts.height || 280) > $('body').height() ? $('body').height() : (opts.height || 280);
		// title
		opts.title = opts.title || '提示';
		// content, should be url if type is either url or ajax
		opts.content = opts.content || '提示信息';
		...
		this.mainWrap = $('<div />').css({
            ...
            'status': 'no',
            'scroll':'no',
			'left': ($(window).width() - this.opts.width) / 2 ,
			'top' : ($(window).height() - this.opts.height) / 2 
		}).appendTo('body').addClass('pop-box');
        
        this.mainWrap2 = $('<div />').appendTo(this.mainWrap).addClass('box');
        
		this.headWrap = $('<div />').appendTo(this.mainWrap2).addClass('box-hd');

		this.titleWrap = $('<h3 />').css({
//			'float': 'left',
//			'width': (this.opts.width - 50) + 'px'
		}).html(this.opts.title).appendTo(this.headWrap).addClass('title');
		...
		this.contentWrapWrap = $('<div />').css({ 
			'display': 'block',
			'width': this.opts.width,
			'height': (this.opts.height - 24)
//			'overflow': 'auto' 
		}).appendTo(this.mainWrap2).addClass('box-bd');
		
		this.contentWrap = $('<div />').appendTo(this.contentWrapWrap).html(this.opts.content).addClass('mod-form');
		
//        this.formWrap = $('<div />').appendTo(this.contentWrap).html(this.opts.content).addClass('mod-form');
	...
	$.msgbox.autoclose = {
		...
		domready: function(){
			...
			var _this = this;
			this.closeMsg = '&nbsp; &nbsp; <strong>{seconds}</strong>s 后自动关闭';
			var interval = null;
		...
		domready: function(){
			...
			switch(this.opts.type){
				...
				case 'confirm':
					...
					var yesbutton = $('<button  class="btn-confirm" type="button" value=" 确定 " ><span>确定</span></button>').click(function(){
						_this.setValue( true );
						_this.close();	
					});
					var nobutton = $('<button  class="btn-cancel" type="button" value=" 取消 " ><span>取消</span></button>').click(function(){
						_this.setValue( false );
						_this.close();	
					});
					var center = $('<div />').append(yesbutton).append('&nbsp;').append(nobutton).addClass('btns');;
					this.contentWrap.append(center);
		...
		domready: function(){
			...
			this.setShowClose = function(sc){
				if( sc ){
                    ...
                    _this.closeWrap.css('margin', '0px');
                    _this.closeWrap.css('top', '0px');
                    _this.closeWrap.css('right', '0px');
                    _this.closeWrap.css('position', 'absolute');
